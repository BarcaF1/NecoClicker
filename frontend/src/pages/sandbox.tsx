...
import { Play, Square, Trash2, FlaskConical, Activity, Gauge, RotateCcw, ShieldCheck } from 'lucide-react'
      ...
      <div className="flex min-h-0 flex-col gap-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-3 gap-2">
              ...
              <Stat label="Всего" value={cps.total.toLocaleString()} />
            </div>
        ...
        <Card className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2 text-sm">
                <Activity className="h-4 w-4 text-primary" /> Лог выполнения
                {log.length > 0 && (
          ...
          <ScrollArea className="flex-1">
            <div ref={scrollRef} className="px-5 pb-4 font-mono text-[11px] leading-relaxed">
              {log.length === 0 ? (
                <div className="flex h-32 items-center justify-center text-xs text-muted-foreground">
                  (пусто) — запусти что-нибудь чтобы увидеть события
                </div>
              ) : (
                log.map((line, i) => (
        ...
        <CardContent className="space-y-3">
          ...
          <div className="rounded-md border border-dashed border-primary/30 bg-primary/5 p-3 text-xs">
            ...
            <div className="text-muted-foreground">
              В безопасном режиме можно прогнать профиль и увидеть реальный CPS без эффекта на систему.
              Для бенчмарка ставь интервал <span className="font-mono text-foreground">0</span> на главной — будет жать максимум.
            </div>
          </div>
        </CardContent>
      </Card>
...
/**
 * BarChart — последние 80 семплов по 250мс ≈ 20 секунд истории CPS.
 * Каждый бар = одна точка измерения. Высота нормирована к max.
 */
    ...
    <div className="relative flex h-[60px] items-end gap-[2px] overflow-hidden rounded-md border border-border bg-card/30 px-2 py-1.5">
      {data.length === 0 ? (
        ...
        Array.from({ length: 80 }).map((_, i) => {
          ...
          const recent = i >= 76 // последние ~1 сек подсветим ярче
          return (
            <div
              key={i}
              className={cn(
